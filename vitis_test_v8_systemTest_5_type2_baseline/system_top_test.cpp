...
int main()
{
    ...
    vx_uint8 lenA = 0;
    vx_uint8 lenC = 0;
    procVecType matA[ROW_LEN];
    procVecType matC[ROW_LEN];
    ...
    for (vx_uint8 mats = begin_mat_num-1; mats < end_mat_num; mats++) {
        ...
        while (!system_ready) {
            ...
            system_start = false;
            cycle_count++;
            //PRINT("\n MAIN: Cycles = %i | access_count = %i", cycle_count, access_count);
        }
        ...
        //printf ("\n\n MAIN: Output[%i] | density = %i | Valid = %i | runtime (nano_sec) = %i \n\n", mats+1, density, valid, runtime.count());
        // Output metrics to file
        string out_str = "Mat_num = " + to_string(mats+1) + "\t\t" + "| Density(%) = " + to_string(density) + "\t\t" + "| Valid(bool) = " + to_string(valid) + "\t\t" + "| cycles = " + to_string(cycle_count) + "\t\t" + "| runtime(mili_sec) = " + to_string(runtime.count()) + "\t\t" + "| memory_access_count = " + to_string(access_count) + "\n";
